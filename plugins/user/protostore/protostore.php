...
//        Cart::destroyCookie();
	...
	private function mopGuestOrders($userid, $user_email)
	{
		...
		{
			$ordersToCheck[] = OrderFactory::get($guestOrder);
		}
		...
		foreach ($ordersToCheck as $order)
		{
			if ($order->billingEmail == $user_email)
			{
				...
				$addressToUpdate = AddressFactory::get($order->billing_address_id);

				if (AddressFactory::setCustomerId($addressToUpdate, $order->customerid))
				{
