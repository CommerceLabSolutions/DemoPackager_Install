...
class plgSystemUniversal_elements extends CMSPlugin
{
	...
	public function onUserAfterSave($data, $isNew, $result, $error): void
	{
	    ...
	    $field_data = (array) json_decode($userData->field_data);
	    if(!empty($field_data)){
    	    foreach($field_data as $key => $value){
    	        $data_key = preg_replace('/[0-9]+/', '', $key);
    	        foreach($data as $key1 => $value1){
        	        if($data_key == $key1){
        	            if($key == "profile"){
        	                 foreach($field_data["profile"] as $k => $val){
        	                     foreach($data["profile"] as $key2 => $value2){
            	                     if($k == $key2){
            	                         $field_data["profile"]->$k = $value2;
            	                     }
        	                     }
        	                 }
        	            }else if($key == "com_fields"){
        	                 foreach($field_data["com_fields"] as $k1 => $val1){
        	                     foreach($data["com_fields"] as $k2 => $v2){
            	                     if($k1 == $k2){
            	                         $field_data["com_fields"]->$k1 = $v2;
            	                     }
        	                     }
        	                 }
        	            }else{
        	                $field_data[$key] = $value1;
        	            }
        	        }
    	        }
    	    }
	    }
	    $final_data = (object) $field_data;
	    $final_json_data = json_encode($final_data);
	    $this->updateUserFieldData($final_json_data,$user_id);
	}
	
	public function updateUserFieldData($final_json_data,$user_id){
	    $db = Factory::getDbo();
        // Create a new query object.
        $query = $db->getQuery(true);
        $fields = array(
            $db->quoteName('field_data') . ' = ' . $db->quote($final_json_data)
        );		
