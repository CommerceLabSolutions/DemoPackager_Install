...
foreach ($extra as $extra_fieldset) { 
    if($child->props['field_type'] == $extra_fieldset->name){
        ...
        if($type == "text" || $type == "textarea" || $type == "password" || $type == "email" || $type == "url" || $type == "tel"){
            ...
            if($child->props['field_hide_value'] && empty($child->props["field_approval_needed"])){
                $placeholder = "******************";
                $field_value = "";
            }
            if($type == "textarea"){
                if(empty($field_value) && !empty($child->props["field_approval_needed"])){
                    ...
                    echo '<textarea class="uk-textarea '.$child->props["fields_size"].'" rows="5" placeholder="'.$placeholder.'" '.$required.' aria-label="Textarea" id="'.$extra_fieldset->name.'" name="jform[com_fields]['.$extra_fieldset->name.']" '.$disabled.'></textarea>';
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"]) && $approved_status == 1){
                    echo '<textarea class="uk-textarea '.$child->props["fields_size"].'" rows="5" placeholder="'.$placeholder.'" '.$required.' aria-label="Textarea" id="'.$extra_fieldset->name.'" name="jform[com_fields]['.$extra_fieldset->name.']" '.$disabled.'>'.$field_value.'</textarea>';
                ...
                if(empty($field_value) && !empty($child->props["field_approval_needed"])){
                    echo '<input type="hidden" name="cust_f['.$extra_fieldset->name.']" value="">';
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$extra_fieldset->name.'" '.$required.' type="'.$type.'" name="jform[com_fields]['.$extra_fieldset->name.']" placeholder="'.$placeholder.'" '.$disabled.'>'; 
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"]) && $approved_status == 1){
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$extra_fieldset->name.'" '.$required.' type="'.$type.'" name="jform[com_fields]['.$extra_fieldset->name.']" placeholder="'.$placeholder.'" value="'.$field_value.'">'; 
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"])){
                    echo '<input type="hidden" name="cust_f['.$extra_fieldset->name.']" value="'.$field_value.'">';
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$extra_fieldset->name.'" '.$required.' type="'.$type.'" name="jform[com_fields]['.$extra_fieldset->name.']" placeholder="'.$placeholder.'" '.$disabled.'>'; 
                }else{
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$extra_fieldset->name.'" '.$required.' type="'.$type.'" name="jform[com_fields]['.$extra_fieldset->name.']" placeholder="'.$placeholder.'" value="'.$field_value.'" '.$disabled.'>'; 
                }
            }
        }else if($type == "radio"){
            ...
            echo '<label><input class="uk-radio '.$child->props["fields_size"].'" type="radio" name="jform[com_fields]['.$extra_fieldset->name.']" value="'.$radio->value.'" '.$check.'>&nbsp;'.$radio->name.'</label><br>';
            $i++;
            }
        }else if($type == "checkboxes"){
            $checkboxs = $extra_fieldset->fieldparams->get('options');
            foreach($checkboxs as $checkbox){
            echo '<label><input class="uk-checkbox '.$child->props["fields_size"].'" type="checkbox" '.$required.' name="jform[com_fields]['.$extra_fieldset->name.'][]" value="'.$checkbox->value.'">&nbsp;'.$checkbox->name.'</label><br>';
            }
        }else if($type == "list"){
            $lists = $extra_fieldset->fieldparams->get('options');                    
            echo '<select class="uk-select '.$child->props["fields_size"].'" name="jform[com_fields]['.$extra_fieldset->name.']" id="form-horizontal-select" '.$required.'>';
