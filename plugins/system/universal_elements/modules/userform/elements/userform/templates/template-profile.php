...
foreach ($profileForm->getFieldsets() as $profile_fieldset) {
    ...
    foreach ($profile_fields as $profile_field) {
        ...
        if($child->props['field_type'] == $field_name){
            if(str_replace("profile","jform[profile]",$profile_field->name) == "jform[profile][dob]"){
                $field_value = !empty($profile_fied_data->{$field_name})?date("Y-m-d", strtotime($profile_fied_data->{$field_name})):'';
                $type = "date";
            }else{
                $field_value = !empty($profile_fied_data->{$field_name})?$profile_fied_data->{$field_name}:'';
                $type = strtolower($profile_field->type);
            }
            ...
            if(strtolower($profile_field->type) == "textarea"){
                if(empty($field_value) && !empty($child->props["field_approval_needed"])){
                    ...
                    echo '<textarea class="uk-textarea '.$child->props["fields_size"].'" rows="5" placeholder="'.$placeholder.'" '.$required.' aria-label="Textarea" id="'.$profile_field->name.'" name="'.str_replace("profile","jform[profile]",$profile_field->name).'" '.$disabled.'></textarea>';
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"]) && $approved_status == 1){
                    echo '<textarea class="uk-textarea '.$child->props["fields_size"].'" rows="5" placeholder="'.$placeholder.'" '.$required.' aria-label="Textarea" id="'.$profile_field->name.'" name="'.str_replace("profile","jform[profile]",$profile_field->name).'" '.$disabled.'>'.$field_value.'</textarea>';
                ...
                if(empty($field_value) && !empty($child->props["field_approval_needed"])){
                    echo '<input type="hidden" name="profile_f['.$field_name.']" value="">';
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$profile_field->name.'" '.$required.' name="'.str_replace("profile","jform[profile]",$profile_field->name).'" type="'.$type.'" placeholder="'.$placeholder.'" '.$disabled.'>';
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"]) && $approved_status == 1){
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$profile_field->name.'" '.$required.' name="'.str_replace("profile","jform[profile]",$profile_field->name).'" type="'.$type.'" placeholder="'.$placeholder.'" value="'.$field_value.'" '.$disabled.'>';
                }else if(!empty($field_value) && !empty($child->props["field_approval_needed"])){
                    echo '<input type="hidden" name="profile_f['.$field_name.']" value="'.$field_value.'">';
                    echo '<input class="uk-input '.$child->props["fields_size"].'" id="'.$profile_field->name.'" '.$required.' name="'.str_replace("profile","jform[profile]",$profile_field->name).'" type="'.$type.'" placeholder="'.$placeholder.'" '.$disabled.'>';
                }else{
