    ...
    foreach ($core_fields as $core_field) {
        if($child->props['field_type'] == $core_field->name){ 
            ...
            if(empty($field_value) && !empty($child->props["field_approval_needed"])){
                ...
                echo '<input class="uk-input '.$child->props["fields_size"].'" '.$required.' id="'.$core_field->name.'" type="'.strtolower($core_field->type).'" name="jform['.$core_field->name.']" placeholder="'.$placeholder.'" '.$disabled.'>';
            }else if(!empty($field_value) && !empty($child->props["field_approval_needed"]) && $approved_status == 1){
                echo '<input class="uk-input '.$child->props["fields_size"].'" '.$required.' id="'.$core_field->name.'" type="'.strtolower($core_field->type).'" name="jform['.$core_field->name.']" placeholder="'.$placeholder.'" value="'.$field_value.'" '.$disabled.'>';
