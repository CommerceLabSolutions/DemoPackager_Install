...
?>
<?= $el($props, $attrs) ?>
<h1 class="<?php echo $props['form_title_margin'].' '.$props['form_title_style'].' '.$props['form_title_size'];?>"><?php echo $props["user_form_title"]?$props["user_form_title"]:"Profile";?></h1>
...
<p class="uk-margin uk-clearfix uk-width-1-1@s <?php echo $props['user_edit_button_margin'];?>'">        
...
<?php }   
if($props['user_submit_button_vertical_alignment'] == "uk-vertical-align-top"){?>  
<p class="uk-margin uk-clearfix uk-width-1-1@s <?php echo $props['user_submit_button_margin'];?>">
...
if($props['user_edit_button_vertical_alignment'] == "uk-vertical-align-bottom" && empty($props['inline_editing_approval'])){?>  
    <p class="uk-margin uk-clearfix uk-width-1-1@s <?php echo $props['user_edit_button_margin'];?>" id="universal_elements_edit_button_section">        
...
<?php } ?>
<?php if($props['user_submit_button_vertical_alignment'] == "uk-vertical-align-bottom"){?>  
<p class="uk-margin uk-clearfix uk-width-1-1@s <?php echo $props['user_submit_button_margin'];?>" id="universal_elements_submit_button_section">
    <button class="uk-button <?php echo $props['user_submit_button_type'].' '.$props['user_submit_button_size']. ' '.$props['user_submit_button_horizontal_placement'];?>" type="submit" id="universal_elements_submit_button"><?php echo $props["user_submit_button_text"]?$props["user_submit_button_text"]:'Save';?></button>    
</p>
<?php } ?>
        ...
        if(valid){
            jQuery.ajax({
                ...
                success:function(response){
                    if(response == "true"){
                        UIkit.notification({message: '<i class="uk-icon-check"></i><?php echo $props['user_form_alert']?>', status: 'success'});
                        setTimeout(function() {
                        location.reload();
                        }, 5000);
                    }
