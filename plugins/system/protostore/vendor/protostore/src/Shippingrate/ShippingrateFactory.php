...
class ShippingrateFactory
{
	...
	public static function getZone(int $id): ?Zoneshippingrate
	{
		...
		$query->select('*');
		$query->from($db->quoteName('#__protostore_zone_shipping_rate'));
		$query->where($db->quoteName('id') . ' = ' . $db->quote($id));
	...
	private static function createFromInputData(Input $data)
	{
		...
		$result = $db->insertObject('#__protostore_shipping_rate', $item);
	...
	private static function zonecreateFromInputData(Input $data)
	{
		...
		$item->modified      = Utilities::getDate();
		$item->modified_by   = Factory::getUser()->id;
		$result = $db->insertObject('#__protostore_zone_shipping_rate', $item);
	...
	private static function zonecommitToDatabase(Zoneshippingrate $item): bool
	{
		...
		$result = $db->updateObject('#__protostore_zone_shipping_rate', $insert, 'id');
	...
	public static function trashFromInputData(Input $data): bool
	{
		...
		foreach ($items as $item)
		{
			...
			$query->delete($db->quoteName('#__protostore_shipping_rate'));
	...
	public static function zonetrashFromInputData(Input $data): bool
	{
		...
		foreach ($items as $item)
		{
			...
			$query->delete($db->quoteName('#__protostore_zone_shipping_rate'));
			$query->where($conditions);
			$db->setQuery($query);
			$db->execute();
	...
	public static function togglePublishedFromInputData(Input $data)
	{
		...
		foreach ($items as $item)
		{
			$query = 'UPDATE ' . $db->quoteName('#__protostore_shipping_rate') . ' SET ' . $db->quoteName('published') . ' = IF(' . $db->quoteName('published') . '=1, 0, 1) WHERE ' . $db->quoteName('id') . ' = ' . $db->quote($item['id']) . ';';
			$db->setQuery($query);
			$result = $db->execute();
	...
	public static function zonetogglePublishedFromInputData(Input $data)
	{
		...
		foreach ($items as $item)
		{
			$query = 'UPDATE ' . $db->quoteName('#__protostore_zone_shipping_rate') . ' SET ' . $db->quoteName('published') . ' = IF(' . $db->quoteName('published') . '=1, 0, 1) WHERE ' . $db->quoteName('id') . ' = ' . $db->quote($item['id']) . ';';
			$db->setQuery($query);
			$result = $db->execute();
