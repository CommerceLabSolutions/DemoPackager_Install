...
class UserModel extends ItemModel
{
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_user_pro')) && (!$user->authorise('core.edit', 'com_user_pro')))
		{
		...
		if ($this->_item === null)
		{
			...
			if ($table && $table->load($id))
			{
				...
				if ($published = $this->getState('filter.published'))
				{
					...
					{
						throw new \Exception(Text::_('COM_USER_PRO_ITEM_NOT_LOADED'), 403);
					}
				}
			...
			if (empty($this->_item))
			{
				throw new \Exception(Text::_('COM_USER_PRO_ITEM_NOT_LOADED'), 404);
			}
		}
	...
	public function getItemIdByAlias($alias)
	{
		...
		{
			$aliasKey   = $this->getAliasFieldNameByView('user');
		}
