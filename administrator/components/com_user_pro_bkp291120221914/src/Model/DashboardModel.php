...
/**
 * Methods supporting a list of Dashboard records.
 *
 * @since  1.0.0
 */
class DashboardModel extends ListModel
{
    ...
    */
     ...
     *
     * @return mixed count of users on success, false on failure.
     */
    ...
    /**
     * Get a count of users last 30 days
     *
     * @return mixed count of users last 30 days on success, false on failure.
     */
    public function getLastThirtyDayUserCount()
    {
        $db = Factory::getDbo();
        $query = $db->getQuery(true)
            ->select(array('u.*'))
            ->from($db->quoteName('#__users', 'u'))
            ->where('registerDate >= NOW() - INTERVAL 30 DAY');
    ...
    /**
     * Get a count of approval fields
     *
     * @return mixed count of users last 30 days on success, false on failure.
     */
    public function getTotalApprovalFieldsCount()
    {
        $db = Factory::getDbo();
        $query = $db->getQuery(true)
            ->select(array('u.*'))
            ->from($db->quoteName('#__user_pro_field_approval', 'u'))
            ->where('u.status = 0');
        try {
            $db->setQuery($query);
            $db->execute();
