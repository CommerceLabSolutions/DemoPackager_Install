 ...
 * @since  1.0.0
 */
class SettingModel extends ListModel
{
    ...
    */
    ...
    public function getTemplateContent(){
        ...
        $query = $db->getQuery(true)
            ->select("*")
            ->from($db->quoteName('#__user_pro_email_setting', 'es'))
            ->where($db->quoteName('es.id') . ' = :id')
    ...
    public function getEmailTemplateData()
    {
        ...
        $query = $db->getQuery(true)
            ->select("*")
            ->from($db->quoteName('#__user_pro_email_setting', 'upal'));
        try {
