...
class UsersModel extends ListModel
{
	...
	*/
    ...
    public function getLogItems()
    {
        ...
        $db = Factory::getDbo();
        $query = $db->getQuery(true)
            ->select(array('upal.user_id', 'upal.log_message', 'upal.created_time','u.name'))
        ...
        $query1 = $db->getQuery(true)
            ->select(array('al.user_id','al.message_language_key', 'al.message', 'al.log_date','u.name'))
            ->from($db->quoteName('#__action_logs', 'al'))
            ->join('LEFT', $db->quoteName('#__users', 'u') . ' ON (u.id = al.user_id)')
            ->where($db->quoteName('al.user_id') . ' = :user_id')
            ->bind(':user_id', $user_id, ParameterType::INTEGER);
        $rows = $db->setQuery($query1)->loadObjectList();
        // Load all actionlog plugins language files
		ActionlogsHelper::loadActionLogPluginsLanguage();

		foreach ($rows as $row)
		{
			$row->log_message = ActionlogsHelper::getHumanReadableLogMessage($row);
			$row->created_time = $row->log_date;
			unset($row->log_date);
			unset($row->message_language_key);
			unset($row->message);
		}
 		$array = array_merge($rows, $result);
        usort($array, function($a, $b) { 
            if (strtotime($a->created_time) < strtotime($b->created_time))
                return 1;
            else if (strtotime($a->created_time) > strtotime($b->created_time))
                return -1;
            else
                return 0;
        });
