...
class FieldapprovalsController extends AdminController
{
	...
	public function changeStatus(){
		...
		if($db->execute()){
			...
			if($status_type == 1){
			    $this->updateStatusLog("approved",$approval_data);
			    ...
			    $this->updateStatusLog("denied",$approval_data,$approval_data->user_id);
	...
	public function updateStatusLog($type, $approval_data){
	    if($type == "approved"){
	        $message = "Admin approved your field (".$approval_data->field_name. ")";
	    }else{
	       $message = "Admin denied your field (".$approval_data->field_name.")";
	    }
	    $db = Factory::getDbo();
		$query = $db->getQuery(true);
        // Insert columns.
        ...
        // Insert values.
        $values = array($db->quote($approval_data->user_id), $db->quote($message), $db->quote(date("Y-m-d H:i:s")));
