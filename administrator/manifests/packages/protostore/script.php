...
class pkg_protostoreInstallerScript
{
	...
	public function postflight($type, $parent)
	{
		...
		if ($type == 'install')
		{
			...
			{
				//insert a starting emails
				$object            = new stdClass();
				$object->id        = 0;
				$object->to        = Factory::getConfig()->get('mailfrom');
				$object->body      = '<p>Hi Admin</p><p>Order, {order_number},  is now pending </p>';
				$object->emailtype = 'confirmed';
				$object->subject   = 'Order {order_number} Confirmed';
				$object->published = 1;
				$db->insertObject('#__protostore_email', $object);

				//insert a starting emails
				$object            = new stdClass();
				$object->id        = 0;
				$object->body      = '<p>Hi {customer_name}</p><p>Thank you for your order! </p>';
				$object->emailtype = 'thankyou';
				$object->subject   = 'Thank you {customer_name}';
				$object->published = 1;
				$db->insertObject('#__protostore_email', $object);
				...
				$object            = new stdClass();
				$object->id        = 0;
				$object->body      = '<p>Hi {customer_name}</p><p>Your order, {order_number} is now confirmed. </p>';
				$object->emailtype = 'confirmed';
				$object->subject   = 'Your order, {order_number} is now confirmed.';
				$object->published = 1;
				$db->insertObject('#__protostore_email', $object);
				...
				$object            = new stdClass();
				$object->id        = 0;
				$object->body      = '<p>Hi {customer_name}</p><p>Your order, {order_number} is now shipped. </p>';
				$object->emailtype = 'shipped';
				$object->subject   = 'Your order, {order_number} is now shipped.';
				$object->published = 1;
				$db->insertObject('#__protostore_email', $object);
				...
				$object            = new stdClass();
				$object->id        = 0;
				$object->body      = '<p>Hi {customer_name}</p><p>Your order, {order_number} is now pending. </p>';
				$object->emailtype = 'pending';
				$object->subject   = 'Your order, {order_number} is now pending.';
				$object->published = 1;
				$db->insertObject('#__protostore_email', $object);
			...
			$plugin          = new stdClass();
			$plugin->type    = 'plugin';
			$plugin->element = 'pro2storeanimation';
			$plugin->folder  = (string) 'system';
			$plugin->enabled = 1;
			$db->updateObject('#__extensions', $plugin, array('type', 'element', 'folder'));
...
//
				...
				$query = "SHOW COLUMNS FROM `#__protostore_country` LIKE 'default'";
				$db->setQuery($query);
				$res = $db->loadResult();
				if (!$res)
				{
					$query = 'ALTER TABLE ' . $db->quoteName('#__protostore_country') . ' ADD COLUMN `default` tinyint(11) DEFAULT \'0\';';
					$db->setQuery($query);
					$db->execute();
				}

				$query = 'ALTER TABLE ' . $db->quoteName('#__protostore_product_option') . ' ADD COLUMN `product_id` int(11) NOT NULL DEFAULT \'0\';';
				$db->setQuery($query);
				$db->execute();
			 ...
			 */
				...
				$query = 'ALTER TABLE ' . $db->quoteName('#__protostore_discount') . ' CHANGE `couponcode` `coupon_code` varchar(255) DEFAULT NULL;';
				$db->setQuery($query);
				$db->execute();
//
//
